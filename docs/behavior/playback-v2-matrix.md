...
| PB-EN-006 | valid seekBeat | mapped transport position; one Arrangement destination step | covered |
...
| PB-EN-010 | explicit forward/backward seek | missedStepPolicy ignored | covered |
...
| PB-EN-013A | invalid Engine constructor input | Sequencer and Arrangement throw TypeError without normalization | covered |
| PB-EN-014 | non-loop finite Project shortened | move to new end and enter ended | covered |
| PB-EN-015 | looping finite Project shortened | modulo position and continue | covered |
...
| PB-EN-022 | Arrangement setLoop changes value | projectLoop updates and one command loopchange emits | covered |
| PB-EN-023 | Arrangement setLoop gets non-boolean | synchronous TypeError and no event | covered |
| PB-EN-024 | Arrangement setLoop gets current value | no-op and no event | covered |
| PB-EN-025 | Arrangement setArrangement succeeds | one strict atomic Project event | covered |
...
| PB-EV-003 | Project value change | affected Envelope resets without retrigger | covered |
| PB-EV-004 | decay Envelope backward seek | old state does not leak after reset | covered |

## React, reduced motion, SSR, and cleanup

| ID | Scenario | Expected result | Status |
| --- | --- | --- | --- |
| PB-RE-001 | hook mount with valid Project | Engine snapshot becomes hook state | covered |
| PB-RE-002 | invalid initial Project | engine null and projectError populated | covered |
| PB-RE-003 | invalid Project hot-swap | old Engine data survives; projectError populated | covered |
| PB-RE-004 | command rejects | transportError populated and Promise rethrows | covered |
| PB-RE-005 | unsolicited playback error | transportError populated | covered |
| PB-RE-006 | later command succeeds | only transportError clears | covered |
| PB-RE-007 | overlapping toggle calls | queued calls evaluate latest execution-time state | covered |
| PB-RE-008 | queued operations | isBusy remains true; pendingOperation is queue head | covered |
| PB-RE-009 | playing position updates | positionRef/onPosition update without component rerender per frame | covered |
| PB-RM-001 | system reduce enabled after mount | sample once then stop rAF | covered |
| PB-RM-002 | reduced mode ordinary steps | no frame or discrete updates | covered |
| PB-RM-003 | reduced mode seek/stop/Project update | one static sample per explicit change | covered |
| PB-RM-004 | reduction disabled | rAF resumes from current logical position | covered |
| PB-SSR-001 | import all public entries in Node | no browser-global access | covered |
| PB-SSR-002 | SSR render hooks/components | no module/render-phase browser access | covered |
| PB-LC-001 | React StrictMode remount | borrowed transport is not disposed | covered |
| PB-LC-002 | unmount during queued operation | no state update after unmount; caller transport survives | covered |
| PB-LC-003 | animation cleanup ordering | rAF/subscriptions detach before Engine reference release | covered |

## Player React

| ID | Scenario | Expected result | Status |
| --- | --- | --- | --- |
| PB-UI-001 | stopped controls | Play enabled; Stop has stable dimensions/state | covered |
| PB-UI-002 | playing controls | Play becomes Pause; Stop remains separate | covered |
| PB-UI-003 | paused controls | Resume and Stop are available | covered |
| PB-UI-004 | pending operation | loading/disabled state reflects isBusy | covered |
| PB-UI-005 | stop | UI and playhead return to position 0 | covered |
| PB-UI-006 | seekPositionMs scrub | long media seeks without limiting to one pattern | covered |
| PB-UI-007 | transport error | visible recoverable error state; layout remains stable | covered |
