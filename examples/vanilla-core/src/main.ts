  ...
  app.innerHTML = `
    <section class="shell">
      <header class="toolbar">
        ...
        <div class="transport">
          <button data-action="toggle">${engine.getPlaybackState() === "playing" ? "Pause" : "Play"}</button>
      ...
      ${transportError ? `<p role="alert">${escapeHtml(transportError)}</p>` : ""}
...
app.addEventListener("click", (event) => {
  ...
  switch (button.dataset.action) {
    case "toggle":
      ...
      break;
    case "stop":
      runTransportCommand(() => engine.stop());
      break;
    case "clear":
